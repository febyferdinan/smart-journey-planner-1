...
interface StopInputProps {
    stopAddress: string;
    setStopAddress: (address: string) => void;
...
export function StopInput({ stopAddress, setStopAddress, provider, placeholder, showLabel = true, googleMapsApiKey }: StopInputProps) {
            ...
            try {
                ...
                if (provider === 'mapbox') {
                    ...
                    const response = await axios.get(
                        `https://api.mapbox.com/geocoding/v5/mapbox.places/${encodeURIComponent(stopAddress)}.json`,
                    ...
                    const predictions = await autocompleteGoogle(stopAddress, googleMapsApiKey);
    ...
    }, [stopAddress, provider]);

    const handleSelectSuggestion = (suggestion: Suggestion) => {
        ignoreNextFetch.current = true;
        setStopAddress(suggestion.value);
        setShowSuggestions(false);
    };
    ...
    return (
        <div className="relative">
            {showLabel && (
                <label htmlFor="stop" className="block text-sm font-medium text-gray-700 dark:text-gray-300">
            ...
            <div className="mt-1 relative">
                <input
                    ...
                    placeholder={placeholder || "Enter intermediate stop (e.g., Home, Office)"}
                />
                {isLoading && (
